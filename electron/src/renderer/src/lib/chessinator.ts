...
export class Chessinator extends Chess {
  ...
  private removePieceBySquare = (square: Square) => {
    this.pieces.forEach((p) => console.log(p.position))

    this.pieces = this.pieces.filter((p) => p.position != square)
    this.derivePieces()
  }

  private removeEatedPieces = () => {
    const localPieces = new Set(this.getLocalPieces().map((p) => p.position))
    this.setPieces(this.pieces.filter((p) => localPieces.has(p.position)))
  }
  ...
  applyMove = (move: LocalMove | string) => {
    ...
    if (moveResult.captured && moveResult.isEnPassant()) {
      this.updatePiecePosition(moveResult.from, moveResult.to)
      this.removeEatedPieces()
      return
    }
