...
export const useProcessFriendRequest = () => {
  return useMutation({
    mutationKey: ['friend'],
    mutationFn: async ({ userId, response }: { response: boolean; userId: User['id'] }) => {
      api.friend['friend-request'].response({ userId }).post({}, { query: { response } })
    },
    onMutate: ({ userId }, _) => {
      const prevRequest = queryClient.getQueryData<FriendRequests[]>(['friend-requests'])
